...
class EventRouter implements Listener{
    ...
    // Save the player's original position before any teleportation (such as to spawn)
    $this->origPos = $player->getLocation(); 
    
    // Optionally log or debug the position to check if it's being saved correctly
     $this->main->getLogger()->debug("Original position saved for player " . $player->getName() . ": " . $this->origPos);
    
    // Start user authentication process (or other processes you have)
    $this->main->startUser($player);
}

    public function onQuit(PlayerQuitEvent $event){
        $player = $event->getPlayer();
        $user = $this->main->getUserByPlayer($player);
        if ($user !== null && $user->getAccountInfo()->opts->maskLoc){
            $originalPos = $this->origPos;
            if ($originalPos !== null){
                $this->main->getLogger()->debug("Restoring player " . $player->getName() . " to their original position: " . $originalPos);
                $player->teleport($originalPos);
            }
        }else{
            $this->main->getLogger()->debug("Player " . $player->getName() . " has completed login or was not teleported due to maskLoc.");
        }
        $this->main->closeUser($player);
    }
...
//				}
	...
	public function onSend(DataPacketSendEvent $event){
		...
		if($pk::NETWORK_ID === Info::CONTAINER_SET_CONTENT_PACKET or $pk::NETWORK_ID === 0xb9){
