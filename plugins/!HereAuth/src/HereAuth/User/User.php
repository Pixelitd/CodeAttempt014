...
class User{
	...
	protected function makeInvisible(){
        ...
        $night = $this->getPlayer()->getEffect(Effect::NIGHT_VISION);
        $blind = $this->getPlayer()->getEffect(Effect::BLINDNESS);
		if($invis !== null){
			$this->revInvis = clone $invis;
            $this->revNight = clone $night;
            $this->revBlind = clone $blind;
	...
	protected function makeVisible(){
		if($this->revInvis !== null){
            ...
            $this->getPlayer()->removeEffect(Effect::NIGHT_VISION);
			if($this->revInvis instanceof Effect and $this->revNight instanceof Effect and $this->reBlind instanceof Effect){
				$this->getPlayer()->addEffect($this->revInvis);
                $this->getPlayer()->addEffect($this->revNight);
                $this->getPlayer()->addEffect($this->revBlind);
			}
			$this->revInvis = null;
		}
	}
