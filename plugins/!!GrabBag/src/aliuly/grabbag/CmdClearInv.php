...
class CmdClearInv extends BasicCli implements CommandExecutor {
	...
	public function onCommand(CommandSender $sender,Command $cmd,$label, array $args) {
		...
		switch ($cmd->getName()) {
			case "clearinv":
				...
				$sender->sendMessage(mc::_("{$target->getName()}'s inventory cleared",$target->getName()));
				return true;
			case "clearhotbar":
				InvUtils::clearHotbar($target);
				if ($other) $target->sendMessage(mc::_("Your hotbar has been cleared by %1%", $sender->getName()));
				$sender->sendMessage(mc::_("{$target->getName()}'s hotbar cleared",$target->getName()));
				return true;
		}
		return false;
	}
	private function rmInvItem(CommandSender $sender, array $args) {
		...
		if ($target->isCreative() || $target->isSpectator()) {
			$sender->sendMessage(mc::_("{$target->getName()} is in %2% mode", $target->getDisplayName(),
