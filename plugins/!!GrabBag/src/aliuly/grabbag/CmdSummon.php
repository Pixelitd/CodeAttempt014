...
class CmdSummon extends BasicCli implements CommandExecutor {
	...
	public function __construct($owner) {
		...
		PermUtils::add($this->owner, "gb.cmd.summonplayer", "summon|dismmiss command", "op");
		$this->enableCmd("summonplayer",
							  ["description" => mc::_("Teleports players to your location"),
								"usage" => mc::_("/summonplayer <player> [message]"),
								"permission" => "gb.cmd.summonplayer"]);
	...
	public function cmdSummon(CommandSender $c,$args) {
		...
		$mv = new Vector3($c->getX()+mt_rand(-3,3),$c->getY(),
								$c->getZ()+mt_rand(-3,3));
		$c->sendMessage(mc::_("Summoning {$pl->getName()}....",$pn));
	...
	public function cmdDismiss(CommandSender $c,$args) {
		...
		foreach ($args as $i) {
			...
			$c->sendMessage(mc::_("Dismissing {$pl->getName()}",$i));
