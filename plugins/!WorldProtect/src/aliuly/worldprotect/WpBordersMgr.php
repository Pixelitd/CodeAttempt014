...
class WpBordersMgr extends BaseWp implements Listener{
	...
	public function onSCommand(CommandSender $c, Command $cc, string $scmd, $world, array $args) : bool{
		...
		if(count($args) === 4){
			...
			$this->owner->getServer()->broadcastMessage(mc::_("[WP] Border for %1% set to (%2%,%3%)-(%4%,%5%)", $world, number_format($x1,2,'.',''), number_format($z1,2,'.',''), number_format($x2,2,'.',''), number_format($z2,2,'.','')));
			return true;
		}
		return false;
	}

	private function checkMove(string $world, float $x, float $z) : bool{
	...
	public function onPlayerMove(PlayerMoveEvent $ev) {
		...
		$pl = $ev->getPlayer();
		$pos = $ev->getTo();
		if($this->checkMove($pl->getLevel()->getFolderName(), $pos->getX(), $pos->getZ())) return;
	...
	public function onTeleport(EntityTeleportEvent $ev) {
		...
		$to = clone $ev->getTo();
		$world = $to->getLevel()->getFolderName();
