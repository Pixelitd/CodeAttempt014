...
use pocketmine\scheduler\PluginCallbackTask;
...
class WpMotdMgr extends BaseWp implements Listener, CommandExecutor{
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $args) {
		...
		if($sender instanceof Player){
			...
			$world = $this->owner->getServer()->getDefaultLevel()->getFolderName();
		}
	...
	private function showMotd(CommandSender $c, string $world) {
	...
	public function onJoin(PlayerJoinEvent $ev) {
		if(!$this->auto) return;
		$pl = $ev->getPlayer();
		$this->showMotd($pl, $pl->getLevel()->getFolderName());
	}

	public function onLevelChange(EntityTeleportEvent $ev) {
		if($ev->getFrom()->getLevel()->getFolderName() !== $ev->getTo()->getLevel()->getFolderName()){
