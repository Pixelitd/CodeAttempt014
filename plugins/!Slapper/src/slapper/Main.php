...
class Main extends PluginBase implements Listener {
					...
					switch ($arg) {
						...
						case "version":
							if($sender->hasPermission("slapper.version") || $sender->hasPermission("slapper")) {
								...
								$sender->sendMessage($this->prefix . TextFormat::BLUE . $desc->getName() . " " . $desc->getVersion() . " " . TextFormat::GREEN . "by " . TextFormat::GOLD . "jojoe77777, Pixelited");
								return true;
							} else {
								$sender->sendMessage($this->noperm);
								return true;
													...
													case "name_visible":
														if(isset($args[2])) {
															switch (strtolower($args[2])) {
																...
																case "always":
																case "1":
																	$entity->namedtag->CustomNameVisible = new ByteTag("CustomNameVisible", 1);
																	$entity->respawnToAll();
																...
																case "lookingat":
																case "onhover":
																	$entity->namedtag->CustomNameVisible = new ByteTag("CustomNameVisible", 2);
																	$entity->respawnToAll();
																	$sender->sendMessage($this->prefix . "Name visibility has been updated.");
																	return true;
																	break;
																...
																case "no":
																case "0":
																	$entity->namedtag->CustomNameVisible = new ByteTag("CustomNameVisible", 0);
																	$entity->respawnToAll();
																	$sender->sendMessage($this->prefix . "Name visibility has been updated.");
																	return true;
																	break;
	...
	private function makeNBT($type, Player $player) {
		...
		$nbt->CustomName = new StringTag("CustomName", $player->getName());
	...
	public function onEntityDamage(EntityDamageEvent $event) {
		...
		if($entity instanceof SlapperEntity || $entity instanceof SlapperHuman) {
			...
			if(isset($entity->namedtag->Commands)) {
				...
				foreach ($entity->namedtag->Commands as $cmd) {
					$server->dispatchCommand(new ConsoleCommandSender(), str_ireplace("{player}", $damagerName, $cmd));
				}
			}
		}
	}
