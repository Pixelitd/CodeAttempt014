...
class ZMusicBox extends PluginBase implements Listener{
	...
	public function onEnable(){
		$this->getLogger()->debug("ZMusicBox is loading!");
		$this->getServer()->getPluginManager()->registerEvents($this, $this);
		if(!is_dir($this->getPluginDir())){
			@mkdir($this->getServer()->getDataPath()."plugins/PlayNoteBlockSong");
		}
		$this->getServer()->getPluginManager()->registerEvents($this,$this);
		if(!$this->CheckMusic()){
			$this->getLogger()->debug("§bPlease put in nbs files!!!");
		}else{
			$this->StartNewTask();
		}
		$this->getLogger()->debug("ZMusicBox loaded!!!!!");
	} 
	...
	public function CheckMusic(){
		if($this->getDirCount($this->getPluginDir()) > 0 and $this->RandomFile($this->getPluginDir())){
			return true;
		}
		return false;
	}
	...
	public function getPluginDir(){
		return $this->getServer()->getDataPath()."plugins/PlayNoteBlockSong/";
	}
	
	public function getRandomMusic(){
		$dir = $this->RandomFile($this->getPluginDir());
		if($dir){
	...
	public function onDisable(){
		$this->getLogger()->debug("ZMusicBox Unload Success!");
	}
