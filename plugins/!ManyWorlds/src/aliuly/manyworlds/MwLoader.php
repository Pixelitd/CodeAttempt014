...
final class MwLoader extends BasicCli{
	public function __construct(BasicPlugin $owner){
		...
		$this->enableSCmd("load", ["usage" => mc::_("<world|--all>"),
			...
			"aliases" => ["ld"]]);
		$this->enableSCmd("unload", ["usage" => mc::_("[-f] <world: string>"),
	...
	private function mwWorldLoadCmd(CommandSender $sender, string $wname): bool {
		if($wname === "--all"){
			$wlst = [];
			...
			$sender->sendMessage(
				TextFormat::AQUA .
				mc::n(mc::_("[MW] Loading one level"), mc::_("[MW] Loading ALL %1% levels", (string)count($wlst)), count($wlst)));
		}else{
	...
	private function mwWorldUnloadCmd(CommandSender $sender, string $wname, bool $force): bool {
