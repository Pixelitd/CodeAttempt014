...
class Session implements Listener{
	 ...
	 * @return mixed
	 */
	public function getState(string $label, Player|string $player, mixed $default): mixed {
	 ...
	 * @return mixed
	 */
	public function setState(string $label, Player|string $player, mixed $val): mixed {
		$player = MPMU::iName($player);
		if(!isset($this->state[$player])){
			$this->state[$player] = [];
		}
	 ...
	 */
	public function unsetState(string $label, Player|string $player){
		$player = MPMU::iName($player);
		if(!isset($this->state[$player])){
			return;
		}
