...
class SleeperHandler
{
	...
	/**
	 * @param \Closure $handler Called when the notifier wakes the server up, of the signature `function()
	 */
	public function addNotifier(SleeperNotifier $notifier, \Closure $handler)
	{
	 ...
	 */
	public function removeNotifier(SleeperNotifier $notifier)
	{
	 ...
	 * back to sleep.
	 */
	public function sleepUntil(float $unixTime)
	{
		while (true) {
	 ...
	 * already waiting.
	 */
	public function sleepUntilNotification()
	{
		$this->threadedSleeper->sleep();
		$this->processNotifications();
	}
