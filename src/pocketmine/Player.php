...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer
{
    ...
    public function handleDataPacket($packet)
    {
        ...
        switch ($packet::NETWORK_ID) {
            ...
            case \pocketmine\network\protocol\p70\Info::ITEM_FRAME_DROP_ITEM_PACKET:
            ...
            case \pocketmine\network\protocol\p70\Info::PLAYER_INPUT_PACKET:
                break;
            case ProtocolInfo::LOGIN_PACKET:
            case \pocketmine\network\protocol\p70\Info::LOGIN_PACKET:
                if ($this->loggedIn) {
                    break;
                }
                $pk = new PlayStatusPacket();
            ...
            case \pocketmine\network\protocol\p70\Info::MOVE_PLAYER_PACKET:
                ...
                if ($this->teleportPosition !== null or ($this->forceMovement instanceof Vector3 and (($newPos->distanceSquared($this->forceMovement)) > 0.1 or $revert))) {
                    if ($this->forceMovement instanceof Vector3) $this->sendPosition($this->forceMovement, $packet->yaw, $packet->pitch, MovePlayerPacket::MODE_RESET);
                } else {
                    $packet->yaw %= 360;
                    $packet->pitch %= 360;
                    if ($packet->yaw < 0) {
            ...
            case \pocketmine\network\protocol\p70\Info::MOB_EQUIPMENT_PACKET:
            ...
            case \pocketmine\network\protocol\p70\Info::USE_ITEM_PACKET:
            ...
            case \pocketmine\network\protocol\p70\Info::PLAYER_ACTION_PACKET:
                if ($this->spawned === false || $this->blocked === true || (!$this->isAlive() && $packet->action !== PlayerActionPacket::ACTION_SPAWN_SAME_DIMENSION && $packet->action !== PlayerActionPacket::ACTION_SPAWN_OVERWORLD)) {
                    break;
                }
                $packet->eid = $this->id;
            ...
            case \pocketmine\network\protocol\p70\Info::REMOVE_BLOCK_PACKET:
            ...
            case \pocketmine\network\protocol\p70\Info::MOB_ARMOR_EQUIPMENT_PACKET:
                break;
            case ProtocolInfo::INTERACT_PACKET:
            case \pocketmine\network\protocol\p70\Info::INTERACT_PACKET:
                if ($this->spawned === false || !$this->isAlive() || $this->blocked) {
                    break;
                }
                $this->craftingType = self::CRAFTING_SMALL;
            ...
            case \pocketmine\network\protocol\p70\Info::ANIMATE_PACKET:
                if ($this->spawned === false || !$this->isAlive()) {
                    break;
                }
            ...
            case \pocketmine\network\protocol\p70\Info::SET_HEALTH_PACKET:
                break;
            ...
            case \pocketmine\network\protocol\p70\Info::DROP_ITEM_PACKET:
                if ($this->spawned === false || $this->blocked === true || !$this->isAlive()) {
                    break;
                }
            ...
            case \pocketmine\network\protocol\p70\Info::TEXT_PACKET:
                if ($this->spawned === false || !$this->isAlive()) {
                    break;
                }
                $this->craftingType = self::CRAFTING_SMALL;
            ...
            case \pocketmine\network\protocol\p70\Info::CONTAINER_CLOSE_PACKET:
            ...
            case \pocketmine\network\protocol\p70\Info::CRAFTING_EVENT_PACKET:
                if ($this->spawned === false || !$this->isAlive()) {
                    break;
                }
            ...
            case \pocketmine\network\protocol\p70\Info::CONTAINER_SET_SLOT_PACKET:
                if ($this->spawned === false || $this->blocked === true || !$this->isAlive()) {
                    break;
                }
                if ($packet->slot < 0) {
            ...
            case \pocketmine\network\protocol\p70\Info::BLOCK_ENTITY_DATA_PACKET:
                if ($this->spawned === false || $this->blocked === true || !$this->isAlive()) {
                    break;
                }
                $this->craftingType = self::CRAFTING_SMALL;
