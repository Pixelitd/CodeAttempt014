...
class RakLibInterface implements ServerInstance, AdvancedSourceInterface
{
	...
	{
		if ($player->getProtocol() == 82 || $player->getProtocol() == 83 || $player->getProtocol() == 84) {
			if (isset($this->identifiers[$h = spl_object_hash($player)])) {
				...
				if (
					!$immediate and !$needACK and $packet::NETWORK_ID !== ProtocolInfo::BATCH_PACKET
			...
			if (isset($this->identifiers[$h = \spl_object_hash($player)])) {
				if ($packet) {
					...
					if (
						!$immediate and !$needACK and $packet::NETWORK_ID !== \pocketmine\network\protocol\p70\Info::BATCH_PACKET
						and Network::$BATCH_THRESHOLD >= 0
						and \strlen($packet->buffer) >= Network::$BATCH_THRESHOLD
					) {
