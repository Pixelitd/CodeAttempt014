...
class Minecart extends Vehicle
{
	 ...
	 */
	private function forwardOnRail(Player $player = null)
	{
		if ($this->direction === -1) {
	...
	private function checkForTurn($currentDirection, $newDirection)
	{
		...
		switch ($currentDirection) {
			case Entity::NORTH:
			case Entity::SOUTH:
				$diff = ($this->x - $this->getFloorX()) - 0.5;
				if (abs($diff) > 0.01) {
					$dx = -$diff * $threshold;
					$this->move($dx, 0, 0);
					return $newDirection;
			...
			case Entity::EAST:
			case Entity::WEST:
				$diff = ($this->z - $this->getFloorZ()) - 0.5;
				if (abs($diff) > 0.01) {
					$dz = -$diff * $threshold;
					$this->move(0, 0, $dz);
					return $newDirection;
				...
				switch ($currentDirection) {
					case Entity::NORTH:
						...
						if ($diff !== 0 and $diff <= 0.5) {
							...
							$this->move($dx, $this->getFloorX() - $this->x - 1, 0);
							return true;
						}
						break;
					case Entity::SOUTH:
				...
				switch ($currentDirection) {
					case Entity::EAST:
						...
						if ($diff !== 0 and $diff <= 0.5) {
							...
							$this->move(0, $this->getFloorZ() - $this->z + 1, $dz);
							return true;
						}
						break;
					case Entity::WEST:
