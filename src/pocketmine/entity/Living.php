...
abstract class Living extends Entity implements Damageable
{
	...
	public function attack($damage, EntityDamageEvent $source)
	{
		...
		if ($source instanceof EntityDamageByEntityEvent) {
			...
			if (!$e->isOnGround() and ($e instanceof Player)) {
				for ($i = 0; $i < 5; $i++) {
					$this->level->addParticle(new CriticalParticle(new Vector3($this->x + mt_rand(-10, 10) / 10, $this->y + mt_rand(5, 10) / 10, $this->z + mt_rand(-10, 10) / 10)));
				}
			}
