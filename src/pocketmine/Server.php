...
class Server
{
	...
	public function about()
	{
		...
		$this->getLogger()->info('PocketMine-MP Antipackets modified by LearXD, Tubunga, xChillz, zStxba & Pixelited');
		$this->getLogger()->info('Accepted protocols: §f[70 ... 84]');
	}
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath, $defaultLang = "unknown")
	{
		...
		try {
			...
			if (!file_exists($this->dataPath . "khronos.yml")) {
				$content = file_get_contents($this->filePath . "src/pocketmine/resources/khronos.yml");
				@file_put_contents($this->dataPath . "khronos.yml", $content);
			}
	...
	public function broadcastPacketsCallback($data, array $identifiers)
	{
		...
		foreach ($identifiers as $i) {
			if (isset($this->players[$i])) {
				if ($this->players[$i]->getProtocol() == 82 || $this->players[$i]->getProtocol() == 83 || $this->players[$i]->getProtocol() == 84) {
	...
	{
		if ($p->getProtocol() == 82 || $p->getProtocol() == 83 || $p->getProtocol() == 84) {
