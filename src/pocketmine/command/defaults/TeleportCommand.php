	...
	public function execute(CommandSender $sender, $currentAlias, array $args)
	{
		...
		if (count($args) === 1 or count($args) === 3) {
			if ($sender instanceof Player) {
				...
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.tp.player"));
			...
			if (count($args) === 1) {
				...
				if ($target === null) {
					$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.player.notFound". " ". $args[0]));
			...
			if (count($args) === 2) {
				...
				if ($target === null) {
					$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.player.notFound". " ". $args[1]));
