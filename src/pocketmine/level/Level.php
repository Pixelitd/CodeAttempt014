	...
	*/
	...
	public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null, bool $createParticles = false): bool
	{
		...
		if ($player !== null) {
			...
			if ($ev->getBlock(Block::FURNACE) or $ev->getBlock(Block::ITEM_FRAME_BLOCK)) {
				$breakTime -= 0.15;
			}

			$breakTime -= 0.05; //1 tick compensation

			if ($this->server->allowInstaBreak and !$ev->getInstaBreak() and ($player->lastBreak + $breakTime) > microtime(true)) {
				return false;
			}
