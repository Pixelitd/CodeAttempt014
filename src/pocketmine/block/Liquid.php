...
abstract class Liquid extends Transparent
{
	...
	public function onUpdate($type)
	{
		...
		// --- FIXED PART BELOW: handles downward flow into same liquid type ---

		$bottomBlock = $this->level->getBlock($this->temporalVector->setComponents($this->x, $this->y - 1, $this->z));

		if (
			$bottomBlock->canBeFlowedInto() ||
			($bottomBlock instanceof Liquid &&
				$bottomBlock->getId() === $this->getId() &&
				$bottomBlock->getDamage() > $decay)
		) {
			// handle lava + water mixing
			...
			$newDecay = ($decay >= 8) ? $decay : ($decay | 0x08);
			$this->flowIntoBlock($bottomBlock, $newDecay);
			return;
		}
