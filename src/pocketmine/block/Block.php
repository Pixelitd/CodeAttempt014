...
class Block extends Position implements BlockIds, Metadatable
{
  ...
  public static function init()
  {
    if (self::$list === null) {
      ...
      self::$list[self::DOUBLE_RED_SANDSTONE_SLAB] =
        DoubleRedSandstoneSlab::class;
      ...
      self::$list[self::LIGHT_WEIGHTED_PRESSURE_PLATE] =
        LightWeightedPressurePlate::class;
      self::$list[self::HEAVY_WEIGHTED_PRESSURE_PLATE] =
        HeavyWeightedPressurePlate::class;
      ...
      self::$list[self::DAYLIGHT_SENSOR_INVERTED] =
        DaylightDetectorInverted::class;
  ...
  public static function get($id, $meta = 0, Position $pos = null)
  {
    if ($id > 0xff) {
      trigger_error(
        "BlockID cannot be higher than 255, defaulting to 0",
        E_USER_NOTICE
      );
      $id = 0;
    }
  ...
  public function canBeFlowedInto()
  {
    ...
    if ($this instanceof Liquid) {
      $block = $this;
      if (get_class($block) === get_class($this) && $block->getDamage() > 0) {
        return true;
      }
    }
    return false;
  ...
  public function getBreakTime(Item $item)
  {
    ...
    if ($this->canBeBrokenWith($item)) {
      if ($this->getToolType() === Tool::TYPE_SHEARS and $item->isShears()) {
        ...
        $this->getToolType() === Tool::TYPE_PICKAXE and
        ($tier = $item->isPickaxe()) !== false or
        $this->getToolType() === Tool::TYPE_AXE and
        ($tier = $item->isAxe()) !== false or
        $this->getToolType() === Tool::TYPE_SHOVEL and
        ($tier = $item->isShovel()) !== false
      ) {
        switch ($tier) {
  ...
  public function getSide($side, $step = 1)
  {
    ...
    return Block::get(
      Item::AIR,
      0,
      Position::fromObject(Vector3::getSide($side, $step))
    );
  }
  ...
  public function calculateIntercept(Vector3 $pos1, Vector3 $pos2)
  {
    ...
    if (
      $v2 !== null and
      ($vector === null or
        $pos1->distanceSquared($v2) < $pos1->distanceSquared($vector))
    ) {
      $vector = $v2;
    }
    ...
    if (
      $v3 !== null and
      ($vector === null or
        $pos1->distanceSquared($v3) < $pos1->distanceSquared($vector))
    ) {
      $vector = $v3;
    }
    ...
    if (
      $v4 !== null and
      ($vector === null or
        $pos1->distanceSquared($v4) < $pos1->distanceSquared($vector))
    ) {
      $vector = $v4;
    }
    ...
    if (
      $v5 !== null and
      ($vector === null or
        $pos1->distanceSquared($v5) < $pos1->distanceSquared($vector))
    ) {
      $vector = $v5;
    }
    ...
    if (
      $v6 !== null and
      ($vector === null or
        $pos1->distanceSquared($v6) < $pos1->distanceSquared($vector))
    ) {
      $vector = $v6;
    }
    ...
    return MovingObjectPosition::fromBlock(
      ...
      $this->z,
      $f,
      $vector->add($this->x, $this->y, $this->z)
    );
  }
  ...
  public function setMetadata($metadataKey, MetadataValue $metadataValue)
  {
    if ($this->getLevel() instanceof Level) {
      $this->getLevel()
        ->getBlockMetadata()
        ->setMetadata($this, $metadataKey, $metadataValue);
    }
  }
  ...
  public function removeMetadata($metadataKey, Plugin $plugin)
  {
    if ($this->getLevel() instanceof Level) {
      $this->getLevel()
        ->getBlockMetadata()
        ->removeMetadata($this, $metadataKey, $plugin);
    }
  }
}
